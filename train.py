...
from gen_captch import number
from gen_captch import alphabet
from gen_captch import ALPHABET
...
def text2vec(text):
    ...
    for i, c in enumerate(text):
        ...
        # 该位字母（数字）所在位置编码为1 其他为0，其实就是one-hot编码
        vector[idx] = 1
    return vector
# 向量转回文本
def vec2text(vec):
    ...
    for i, c in enumerate(char_pos):
        ...
        # 小于10 数字，加上‘0’的ascii编码，即该数字的ascii编码
        if char_idx < 10:
...
# 3 conv layer
    ...
    # dense = tf.reshape(conv3, [-1, fltten_1])
